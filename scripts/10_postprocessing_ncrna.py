...
def process_file(ncrna_tab, out_portrait, out_tpredrna, out_trnascan, out_snoscan, out_rnacon, out_ncrna_tab):
    ...
    # Extraction should start immediately ">" until ":" second group captures the decimal value after the ":"
    ...
    # Processes each line separated by two or more spaces or tabs
    data = []
    for line in lines:
        parts = re.split(r'\s{2,}|\t+', line)
        while len(parts) < 3:
            parts.append(None)
        data.append(parts[:3])  #Max three columns
